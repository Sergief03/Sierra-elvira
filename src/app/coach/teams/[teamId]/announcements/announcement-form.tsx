...
export function AnnouncementForm({ teamId }: { teamId: string }) {
  ...
  return (
    <form action={formAction} className="bg-surface-container-low border border-outline-variant rounded-lg p-md flex flex-col gap-4 max-w-[32rem]">
      <div className="flex items-center gap-xs">
        ...
        <h3 className="font-display text-headline-md text-on-surface">Comunicado al Equipo</h3>
      </div>
      <p className="font-sans text-label-sm text-on-surface-variant">Enviar notificación directa a todos los miembros del equipo.</p>
      <div className="flex flex-col gap-1.5">
        <label htmlFor="title" className="font-sans text-label-bold text-on-surface">Título</label>
        <input id="title" name="title" type="text" required placeholder="Asunto del comunicado" className="input-field w-full text-body-md" />
      </div>
      <div className="flex flex-col gap-1.5">
        <label htmlFor="content" className="font-sans text-label-bold text-on-surface">Mensaje</label>
        <textarea
          id="content"
          name="content"
          required
          ...
          className="input-field w-full text-body-md resize-none"
        />
      </div>
